...
void Read_Data(Do_vat ds_do_vat[], int *T, int *n){
	...
	fscanf(f,"%d", T);
	while(!feof(f)){
		fscanf(f,"%f%f%[^\n]",&ds_do_vat[i].Trong_luong, &ds_do_vat[i].Gia_tri, &ds_do_vat[i].Ten);
		ds_do_vat[i].PA=0;
...
void Print_Data(Do_vat a[], int n, int t){
	...
	printf("|%-3s|%-8s|%-8s|%-8s|%-20s|","STT","TrongLuong", "GiaTri", "DongGia","Ten");
	if(t==1) printf("%8s|", "SoLuong");
	printf("\n");
	int i;
	for(i=0; i<n;i++){
			printf("|%3d|%10.f|%8.f|%8.2f|%-20s|",i+1,a[i].Trong_luong,a[i].Gia_tri,a[i].Don_gia,a[i].Ten);
			if(t==1){
				printf("%-8d|", a[i].PA);	
